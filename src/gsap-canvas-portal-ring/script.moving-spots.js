...
gsap.ticker.add(() => {
  ...
  SPOTS.forEach(({originX, originY, sparks}) => {
    ...
    sparks.forEach(spark => {
      CONTEXT.fillRect(CENTER.x + originX + spark.x, CENTER.y + originY, 5, 5)
...
SPOTS.forEach(({ sparks }) => {
  gsap.to(sparks, {
    x: (index) => sparks[index].travel,
    y: (index) => sparks[index].travel,
    duration: (index) => sparks[index].speed,
    repeat: -1,
    yoyo: true
  })
})